...
public class AdminController {
    ...
    private TextField militaryDiscountField;
    private TextField updateOwnerField;
    private TextField updateRepoField;
    private CheckBox updateAutoCheckBox;
    private com.tireshop.service.UpdateService.UpdateInfo pendingUpdate;
    private TextField ownerEmailField;
    private TextField reportTimeField;
    private CheckBox reportEnabledCheckBox;
    ...
    private ScrollPane createGeneralSettingsPane() {
        ...
        TitledPane updatesPane = createUpdatesPane();

        TitledPane emailReportsPane = createEmailReportsPane();
        ...
        settingsContainer.getChildren().addAll(titleLabel, companyInfoPane, financialSettingsPane, inventorySettingsPane, emailReportsPane, updatesPane, buttonBox);
    ...
    private GridPane createFinancialSettingsGrid() {
        ...
        creditCardFeeField = new TextField(); // Keep field for backward compatibility, but don't display
        globalLowStockThresholdField = new TextField();
        militaryDiscountField = new TextField();
        ...
        grid.add(new Label("Global Low Stock Threshold:"), 0, 1); grid.add(globalLowStockThresholdField, 1, 1);
        grid.add(new Label("Military Discount (%):"), 0, 2); grid.add(militaryDiscountField, 1, 2);
        Label militaryHelp = new Label("Applied with the 🎖 Military Discount button on a pending sale. Set to 0 to disable.");
        militaryHelp.setStyle("-fx-font-size: 11px; -fx-text-fill: #666;");
        grid.add(militaryHelp, 1, 3);
        return grid;
    }
    ...
    /**
     * Updates settings pane - check GitHub Releases for a newer version and
     * install it with automatic backup/rollback.
     */
    private TitledPane createUpdatesPane() {
        GridPane grid = new GridPane();
        grid.setHgap(10); grid.setVgap(10); grid.setPadding(new Insets(10));

        updateOwnerField = new TextField();
        updateOwnerField.setPromptText("e.g. breezy123");
        updateRepoField = new TextField();
        updateRepoField.setPromptText("e.g. tireshop-pos");
        updateAutoCheckBox = new CheckBox("Automatically check for updates when the app starts");

        com.tireshop.service.UpdateService updateService = new com.tireshop.service.UpdateService(SettingsService.getInstance());

        Label versionLabel = new Label(updateService.getCurrentVersion());
        versionLabel.setStyle("-fx-font-weight: bold;");

        Label statusLabel = new Label("Click \"Check for Updates\" to see if a newer version is available.");
        ...
        Button installButton = new Button("⬇ Install Update & Restart");
        installButton.setStyle("-fx-background-color: #27ae60; -fx-text-fill: white;");
            ...
            new Thread(() -> {
                try {
                    ...
                    Platform.runLater(() -> {
                        ...
                        if (info == null) {
                            pendingUpdate = null;
                            statusLabel.setText("✅ You're up to date (version " + updateService.getCurrentVersion() + ").");
                            statusLabel.setStyle("-fx-text-fill: #27ae60;");
                        } else {
                            pendingUpdate = info;
                            String sizeText = info.size > 0 ? String.format(" (%.1f MB)", info.size / 1048576.0) : "";
                            statusLabel.setText("⬇ Update available: version " + info.version + sizeText
                                    + "\nClick \"Install Update & Restart\" to update now.");
        ...
        installButton.setOnAction(e -> {
            ...
            if (!updateService.isRunningFromJar()) {
                showAlert(Alert.AlertType.WARNING, "Not Available Here",
                        "Updates can only be installed when running from the installed application (not in a development environment).");
                return;
            }

            Alert confirm = new Alert(Alert.AlertType.CONFIRMATION);
            confirm.setTitle("Install Update");
            confirm.setHeaderText("Update to version " + pendingUpdate.version + "?");
            confirm.setContentText("The app will download the update, back up the current version, "
                    + "then close and restart itself.\n\n"
                    + "If the new version fails to start, the previous version is restored automatically.");
            ...
            new Thread(() -> {
                try {
                    java.nio.file.Path downloaded = updateService.download(pendingUpdate,
                            p -> Platform.runLater(() -> progressBar.setProgress(p)));

                    Platform.runLater(() -> statusLabel.setText("Download complete. Preparing update..."));
                    java.nio.file.Path backup = updateService.applyUpdateAndRestart(downloaded, pendingUpdate.version);

                    Platform.runLater(() -> {
                        Alert done = new Alert(Alert.AlertType.INFORMATION);
                        done.setTitle("Update Ready");
                        done.setHeaderText("The app will now close and update to version " + pendingUpdate.version);
                        done.setContentText("Current version backed up to:\n" + backup
                                + "\n\nIf the new version fails to start, the backup is restored automatically.");
                        done.initOwner(stage);
                        done.showAndWait();
                        Platform.exit();
                        // Hard exit fallback in case background threads keep the JVM alive
                        new Thread(() -> {
        ...
        int row = 0;
        grid.add(new Label("Current Version:"), 0, row); grid.add(versionLabel, 1, row++);
        grid.add(new Label("GitHub Owner:"), 0, row); grid.add(updateOwnerField, 1, row++);
        grid.add(new Label("GitHub Repository:"), 0, row); grid.add(updateRepoField, 1, row++);
        grid.add(updateAutoCheckBox, 1, row++);
        HBox updateButtons = new HBox(10, checkButton, installButton);
        grid.add(updateButtons, 1, row++);
        grid.add(progressBar, 1, row++);
        grid.add(statusLabel, 1, row);

        TitledPane pane = new TitledPane("🔄 Updates", grid);
    ...
    /**
     * Email & Reports pane - SMTP settings for outgoing mail plus the daily
     * owner sales report (scheduled automatically + manual send button).
     */
    private TitledPane createEmailReportsPane() {
        GridPane grid = new GridPane();
        grid.setHgap(10); grid.setVgap(10); grid.setPadding(new Insets(10));

        ownerEmailField = new TextField();
        ownerEmailField.setPromptText("owner@example.com");
        reportTimeField = new TextField();
        reportTimeField.setPromptText("18:00");
        reportTimeField.setPrefWidth(80);
        reportEnabledCheckBox = new CheckBox("Automatically email the report every day at the time above");

        emailHostField = new TextField();
        emailPortField = new TextField();
        emailPortField.setPrefWidth(80);
        emailUsernameField = new TextField();
        ...
        Button sendNowButton = new Button("📧 Send Today's Report Now");
        sendNowButton.setStyle("-fx-background-color: #2980b9; -fx-text-fill: white;");
        sendNowButton.setOnAction(e -> {
            sendNowButton.setDisable(true);
            statusLabel.setText("Sending report...");
            statusLabel.setStyle("-fx-text-fill: #7f8c8d;");
            new Thread(() -> {
                try {
                    SettingsService settings = SettingsService.getInstance();
                    com.tireshop.dao.SaleDao saleDao = new com.tireshop.dao.SaleDao(DatabaseManager.getSessionFactory());
                    com.tireshop.dao.SalePaymentDao salePaymentDao = new com.tireshop.dao.SalePaymentDao(DatabaseManager.getSessionFactory());
                    com.tireshop.dao.CustomerDao customerDao = new com.tireshop.dao.CustomerDao(DatabaseManager.getSessionFactory());
                    com.tireshop.service.EmailService emailService = new com.tireshop.service.EmailService(settings);
                    com.tireshop.service.DailyReportService reportService = new com.tireshop.service.DailyReportService(
                            settings, saleDao, salePaymentDao, customerDao, emailService);

                    boolean sent = reportService.sendDailyReport(LocalDate.now());
                    Platform.runLater(() -> {
                        sendNowButton.setDisable(false);
                        if (sent) {
                            statusLabel.setText("✅ Report sent to " + settings.getSetting(
        ...
        Label gmailHint = new Label("For Gmail: use your Gmail address as username and an App Password "
                + "(Google Account → Security → App Passwords), not your normal password.");
        gmailHint.setStyle("-fx-font-size: 11px; -fx-text-fill: #666;");
        gmailHint.setWrapText(true);
        gmailHint.setMaxWidth(460);

        int row = 0;
        Label reportHeader = new Label("Daily Owner Report");
        reportHeader.setStyle("-fx-font-weight: bold;");
        grid.add(reportHeader, 0, row++);
        grid.add(new Label("Owner Email:"), 0, row); grid.add(ownerEmailField, 1, row++);
        grid.add(new Label("Send Time (24h):"), 0, row); grid.add(reportTimeField, 1, row++);
        grid.add(reportEnabledCheckBox, 1, row++);
        grid.add(sendNowButton, 1, row++);
        grid.add(statusLabel, 1, row++);
        Label smtpHeader = new Label("Outgoing Email (SMTP)");
        smtpHeader.setStyle("-fx-font-weight: bold;");
        grid.add(smtpHeader, 0, row++);
        grid.add(new Label("SMTP Host:"), 0, row); grid.add(emailHostField, 1, row++);
        grid.add(new Label("SMTP Port:"), 0, row); grid.add(emailPortField, 1, row++);
        grid.add(new Label("Email Username:"), 0, row); grid.add(emailUsernameField, 1, row++);
        grid.add(new Label("Email Password:"), 0, row); grid.add(emailPasswordField, 1, row++);
        grid.add(new Label("From Address:"), 0, row); grid.add(emailFromField, 1, row++);
        grid.add(gmailHint, 1, row);

        TitledPane pane = new TitledPane("📧 Email & Reports", grid);
        pane.setCollapsible(false);
        pane.getStyleClass().add("settings-card");
        return pane;
    }
    ...
    private void loadSettingsIntoUI() {
        ...
        // Display tax rate as percentage (multiply by 100 for display)
        BigDecimal taxRatePercent = settingsService.getSalesTaxRate().multiply(BigDecimal.valueOf(100));
        salesTaxRateField.setText(taxRatePercent.toPlainString());
        // Credit card fee removed - set to zero for backward compatibility
        creditCardFeeField.setText("0.00");
        globalLowStockThresholdField.setText(String.valueOf(settingsService.getGlobalLowStockThreshold()));
        militaryDiscountField.setText(settingsService.getMilitaryDiscountPercent().stripTrailingZeros().toPlainString());
        tireApiLookupEnabledCheckBox.setSelected(settingsService.isTireApiLookupEnabled());
        updateOwnerField.setText(settingsService.getSetting(com.tireshop.service.UpdateService.KEY_OWNER, ""));
        updateRepoField.setText(settingsService.getSetting(com.tireshop.service.UpdateService.KEY_REPO, ""));
        updateAutoCheckBox.setSelected(Boolean.parseBoolean(
                settingsService.getSetting(com.tireshop.service.UpdateService.KEY_AUTO_CHECK, "true")));

        // Email & report settings
        ownerEmailField.setText(settingsService.getSetting(com.tireshop.service.DailyReportService.KEY_RECIPIENT, ""));
        reportTimeField.setText(settingsService.getSetting(com.tireshop.service.DailyReportService.KEY_TIME, "18:00"));
        reportEnabledCheckBox.setSelected(Boolean.parseBoolean(
                settingsService.getSetting(com.tireshop.service.DailyReportService.KEY_ENABLED, "true")));
        emailHostField.setText(settingsService.getSetting(com.tireshop.service.EmailService.KEY_SMTP_HOST, ""));
        emailPortField.setText(settingsService.getSetting(com.tireshop.service.EmailService.KEY_SMTP_PORT, "587"));
        emailUsernameField.setText(settingsService.getSetting(com.tireshop.service.EmailService.KEY_SMTP_USERNAME, ""));
        emailPasswordField.setText(settingsService.getSetting(com.tireshop.service.EmailService.KEY_SMTP_PASSWORD, ""));
        emailFromField.setText(settingsService.getSetting(com.tireshop.service.EmailService.KEY_FROM, ""));
    }
    ...
    private void saveAllSettings() {
        try {
            ...
            // Credit card fee always set to zero - handled by external card machine
            settingsService.setCreditCardFeePercentage(BigDecimal.ZERO);
            ...
            // Military discount percent (whole number, e.g. 10 = 10%); clamp to 0-100
            BigDecimal militaryPercent = new BigDecimal(militaryDiscountField.getText().trim());
            if (militaryPercent.compareTo(BigDecimal.ZERO) < 0 || militaryPercent.compareTo(BigDecimal.valueOf(100)) > 0) {
                showAlert(Alert.AlertType.ERROR, "Invalid Input", "Military discount must be between 0 and 100.");
                return;
            }
            settingsService.setMilitaryDiscountPercent(militaryPercent);

            // Update (auto-update) settings
            settingsService.setSetting(com.tireshop.service.UpdateService.KEY_OWNER, updateOwnerField.getText().trim());
            settingsService.setSetting(com.tireshop.service.UpdateService.KEY_REPO, updateRepoField.getText().trim());
            settingsService.setSetting(com.tireshop.service.UpdateService.KEY_AUTO_CHECK, String.valueOf(updateAutoCheckBox.isSelected()));

            // Email & daily report settings - validate the time format first
            String reportTime = reportTimeField.getText().trim();
            try {
                java.time.LocalTime.parse(reportTime);
            } catch (java.time.format.DateTimeParseException dtpe) {
                showAlert(Alert.AlertType.ERROR, "Invalid Input", "Report send time must be in 24-hour HH:mm format (e.g. 18:00).");
                return;
            }
            settingsService.setSetting(com.tireshop.service.DailyReportService.KEY_RECIPIENT, ownerEmailField.getText().trim());
            settingsService.setSetting(com.tireshop.service.DailyReportService.KEY_TIME, reportTime);
            settingsService.setSetting(com.tireshop.service.DailyReportService.KEY_ENABLED, String.valueOf(reportEnabledCheckBox.isSelected()));
            ...
            showAlert(Alert.AlertType.ERROR, "Invalid Input", "Please enter valid numbers for tax percentage.");
        } catch (Exception e) {
