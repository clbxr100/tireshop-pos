...
public class SaleItem {
    ...
    @Column(name = "quantityReturned", nullable = false, columnDefinition = "int default 0")
    private int quantityReturned = 0; // Track partial returns
    ...
    public void calculateSubtotal() {
        ...
        // Rows loaded from older databases may have a NULL discount - treat as zero
        ...
        this.subtotal = grossAmount.subtract(safeDiscount);
    }
    
    /**
     * Determines if this sale item is taxable based on its type
     * @return true if the item is taxable, false otherwise
     */
    public boolean isTaxable() {
        switch (itemType) {
            case "PRODUCT":
                // Products are generally taxable
                return true;
            case "SERVICE":
                // Services are taxable based on their taxable property
                return service != null ? service.isTaxable() : true;
            case "CUSTOM":
                // Custom items are generally taxable
                return true;
            default:
                return true; // Default to taxable for unknown types
        }
    }
