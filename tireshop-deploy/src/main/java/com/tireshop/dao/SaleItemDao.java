...
public class SaleItemDao extends HibernateDao<SaleItem, Long> {
    ...
    public List<SaleItem> findProductItemsBySaleDateRange(LocalDate fromDate, LocalDate toDate) {
        try (Session session = sessionFactory.openSession()) {
            ...
            // Fetches SaleItems of type PRODUCT where the associated Sale's timestamp is within the range
            Query<SaleItem> query = session.createQuery(
                "FROM SaleItem si WHERE si.itemType = :itemType AND si.sale.timestamp >= :startOfDay AND si.sale.timestamp <= :endOfDay", 
                SaleItem.class);
